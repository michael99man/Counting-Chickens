public class Fighter {
    ...
    public static final int WEALTH_CEILING = 2000000;
    ...
    public static void testBasicStrategy(int nTrials){
        int initialWealth = 200000;

        // test bet sizes in 1k increments up to 10k
        for(int betSize = 1000; betSize<= 10000; betSize+=1000) {
            int hitMin = 0;
    ...
    public static int runBasicStrategy(int wealth, int defaultBetSize){
        ...
        while (wealth >= 0 && wealth <= WEALTH_CEILING){
            int bet = (int) Math.min(defaultBetSize * (1.00 + 0.3*(cock.level - 51)), wealth);
